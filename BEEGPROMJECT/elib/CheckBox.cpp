...
gui::CheckBox::CheckBox(sf::Font& font, std::string label, float scale)
{
	status = false;
...
bool gui::CheckBox::getStatus()
{
	return status;
}

void gui::CheckBox::EventHandler(sf::Event e, const sf::RenderWindow& window)
{
...
void gui::CheckBox::setPosition(const sf::Vector2f& pos)
{
	box.setPosition(pos);
	adjustPositions();
}

sf::Vector2f gui::CheckBox::getSize() const
{
	return box.getSize();
}

void gui::CheckBox::render(sf::RenderTarget& renderTarget)
{
...
void gui::CheckBox::adjustPositions()
{
